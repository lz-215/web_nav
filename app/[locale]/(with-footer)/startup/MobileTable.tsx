...
export default function MobileTable() {
  ...
  return (
    <div className='mb-10 space-y-1 lg:hidden'>
      <div className='flex h-8 items-center justify-between bg-muted px-3 text-xs font-bold text-muted-foreground'>
        <div>{t('website')}</div>
        <div>{t('submission')}</div>
      </div>
      <div className='flex flex-col gap-1'>
        {STARTUP_LIST.map((item) => (
          <div key={item.DA} className='flex min-h-[99px] items-center justify-between bg-card p-3'>
            <div className='flex flex-1 flex-col'>
              <div className='mb-[9px] text-sm text-card-foreground'>
            ...
            <a
              ...
              target='_blank'
              rel='noreferrer'
              className='flex-center h-[75px] w-9 rounded-[4px] border border-border hover:opacity-80'
            >
